...
from pwnlib.util.packing import pack, _need_bytes
...
def update_var(l: int) -> dict[str, int]:
    r"""
    Since different members of the file structure have different sizes,
    we need to keep track of the sizes. The following function is used
    by the FileStructure class to initialise the lengths of the various fields.
    ...
    Return Value:
        Returns a dictionary in which each field is mapped to its corresponding
        length according to the architecture set
...
class FileStructure(object):
    r"""
    Crafts a FILE structure, with default values for some fields, like _lock
    which should point to null ideally, set.

    Arguments:
        null(int)
            A pointer to NULL value in memory. This pointer can lie in any
            segment (stack, heap, bss, libc etc)

    Examples:

        FILE structure with flags as 0xfbad1807 and _IO_buf_base and _IO_buf_end
        pointing to 0xcafebabe and 0xfacef00d
        ...
        The definition for __repr__ orders the structure members and displays
        then in a dictionary format. It's useful when viewing a structure
        object in python/IPython shell.
    ...
    def __setattr__(self, item: str, value: int | bytes | str):
        if item in self._vars:
            maxsize = self.length[item]
            if isinstance(value, int):
                if value < 0 or value >= (1 << maxsize):
                    log.error(f"Out of bounds for {item}: expect item size {maxsize}, but get {value}")
            elif isinstance(value, (str, bytes)):
                value = _need_bytes(value)
                if len(value) > maxsize:
                    log.error(f"Value too large for {item}: expect item size {maxsize}, but get {len(value)}")
            elif hasattr(value, '__bytes__') is False:
                log.error(f"Unable to cast {item} to bytes")
                # if value can cast to bytes, we don't check its size here
                # as it may be mutable
            object.__setattr__(self, item, value)
        elif item in FileStructure.__dict__:
            object.__setattr__(self, item, value)
        else:
    ...
    def __repr__(self) -> str:
        ...
        for i in self.vars_:
            ...
            if isinstance(e, bytes):
                structure.append(f" {i}, {e}")
            elif isinstance(e, int):
                structure.append(f" {i}, {e:#x}")
            else:
                structure.append(f" {i}, {bytes(e)}")
        return "{"+ "\n".join(structure)+"}"
    ...
    def __bytes__(self) -> bytes:
        ...
        for val in self.vars_:
            if self.length[val] > 0:
                ...
                if isinstance(value, int):
                    structure.append(pack(value, self.length[val] * 8))
                elif isinstance(value, bytes):
                    structure.append(value.ljust(self.length[val], b'\x00'))
                else:
                    cast = bytes(value)
                    if len(cast) > self.length[val]:
                        log.error(f"Unable to fit {value} as it's larger than {self.length[val]}")
                    structure.append(cast.ljust(self.length[val], b'\x00'))
        return bytes(structure)

    def struntil(self, v) -> str:
        r"""
        ...
        for val in self.vars_:
            ...
            if val == v:
                break
        return bytes(structure)[:-1]

    def setdefault(self, null: int):
            self.flags = 0
            self._IO_read_ptr = 0
            self._IO_read_end = 0
    ...
    def write(self, addr: int=0, size: int=0) -> bytes:
        r"""
    ...
    def read(self, addr: int=0, size: int=0) -> bytes:
        r"""
