...
HASHES = ['buildid', 'sha1', 'sha256', 'md5']
DEBUGINFOD_SERVERS = [
    ...
    try:
        while not data.startswith(b'\x7fELF'):
            ...
            if data.startswith(b'..'):
                libs_id = Path(data.decode()).parts[3]
...
def find_local_libc(params):
    if not context.local_libcdb:
        log.warn_once("Before enable offline search, please configure `PWNLIB_LOCAL_LIBCDB` or `context.local_libcdb`.")
        return None

    if not params.get("symbols"):
        return None

    db_path = Path(context.local_libcdb)
...
def offline_provider_libc_database(hex_encoded_id, hash_type):
    ...
    if hash_type == "buildid":
        hashfilehex = _get_elf_build_id
    elif hash_type in HASHES:
        hashfilehex = globals()[hash_type + "filehex"]
    else:
...
def search_by_hash(hex_encoded_id, hash_type='buildid', unstrip=True, offline=False):
...
def _search_debuginfo_by_hash(base_url, hex_encoded_id):
    ...
    cache, cache_valid = _check_elf_cache('libcdb_dbg', hex_encoded_id, 'buildid')
    if cache_valid:
        return cache
    ...
    # Save whatever we got to the cache
    write(cache, data or b'')
    shutil.copyfile(cache.replace("_dbg", "") + ".id", cache + ".id")
...
def _check_elf_cache(cache_type, hex_encoded_id, hash_type):
    ...
    >>> cache, _ = _check_elf_cache('libcdb', '2d1c5e0b85cb06ff47fa6fa088ec22cb6e06074e', 'buildid')
    >>> os.unlink(cache) if os.path.exists(cache)
    >>> filename = search_by_hash('2d1c5e0b85cb06ff47fa6fa088ec22cb6e06074e', 'buildid', unstrip=False)
...
def search_by_symbol_offsets(symbols, select_index=None, unstrip=True, return_as_list=False, raw=False, offline=False):
    ...
    Arguments:
        ...
        raw(bool):
            Return a list of full search result.
        offline(bool):
    ...
    Examples:
        ...
        True
        >>> context.local_libcdb = "/home/pwntools/pwntools/pwnlib/data/elf/libcdb/"
    ...
    offline_id = [x["id"] for x in offline_matching] if offline_matching else []
    online_id = [x["id"] for x in online_matching] if online_matching else []

    # Combine offline and online search results.
    matching_id = []
    matching_libcs = []
    ...
    if len(matching_libcs) == 1:
        matched_libc = matching_libcs[0]

    if select_index is not None and matched_libc is None:
        if select_index > 0 and select_index <= len(matching_libcs):
            matched_libc = matching_libcs[select_index - 1]
        else:
    ...
    # Prioritize using the offline libcdb.
    if matched_libc["id"] in offline_id:
        cache, cache_valid = _check_elf_cache('libcdb', matched_libc['buildid'], 'buildid')
        if not cache_valid:
            db_path = Path(context.local_libcdb)
            libc_path = os.path.join(db_path, "%s.so" % matched_libc["id"])
            write(cache, read(libc_path) or b"")
        ...
        return cache
    elif matched_libc["id"] in online_id:
        return search_by_build_id(matched_libc['buildid'], unstrip, offline=False)
    else:
        return None
...
def _get_libc_symbols(path):
    """
    Given symbol file path, the content format is as follows:
        ...
        puts 000xxxxx
        read 0000xxxx
        write 0000xxxx
    ...
    Returns:
        a dictionary of the form: { name : address }, or :const:`{}`
    """
...
def _pack_libs_info(path, libs_id, libs_url, syms):
    ...
    info["buildid"] = _get_elf_build_id(path)
