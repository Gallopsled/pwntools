...
def dealarm_shell(tube):
    ...
    tube.sendline(b'which python || echo')
    if tube.recvline().startswith(b'/'):
        tube.sendline(b'''exec python -c "import signal, os; signal.alarm(0); os.execl('$SHELL','')"''')
        return tube

    tube.sendline(b'which perl || echo')
    if tube.recvline().startswith(b'/'):
        tube.sendline(b'''exec perl -e "alarm 0; exec '${SHELL:-/bin/sh}'"''')
        return tube
