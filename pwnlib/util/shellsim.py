...
For example to create a such a simulated shell over a Tube, you can do this:
    ...
    print sh.e("ls")
    # switch to fancy interactive mode
    sh.interactive()
...
For convenience there is also a client for webshells:

    ws = WebShellClient('http://vuln.example.com/ws.php', "cmd")
    # this will return the content of 'http://vuln.example.com/ws.php?cmd=ls'
    print ws.command("ls")
    # switch to interactive mode with nice prompt, all commands are handled
    # similar to the "ls" above.
    ws.interactive()

A very useful method for CTF scenarios is the following method:

    sh = ShellSim(something)
    # this will use the find command to print all the file that match the
    # regex and print them.
    sh.print_all_files_like(".*flag.*", "/home/ctf/")

If you want to dump the source or in general files:
