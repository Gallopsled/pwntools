...
class FileStructure(object):
    ...
    Examples:
        ...
        Bytes fields are padded to the correct field size, not the pointer size.
        For example, unknown2 is 48 bytes on amd64, so a short value gets
        zero-padded to 48, not to context.bytes (8):
        ...
        >>> len(data)
        224
        >>> fileStr2.length['unknown2']
        48

        Verify the unknown2 field is padded to its full 48-byte size:

        >>> offset = sum(fileStr2.length[v] for v in fileStr2.vars_[:fileStr2.vars_.index('unknown2')])
        >>> field_bytes = data[offset:offset+48]
        >>> field_bytes == b'AB' + b'\x00' * 46
        True
