...
def debug(args, gdbscript=None, exe=None, ssh=None, env=None, sysroot=None, **kwargs):
    ...
    and launches radare2 to attach to it.

    Arguments:
        args(list): Arguments to the process, similar to :class:`.process`.
        gdbscript(str): radare2 script to run.
    ...
    if isinstance(args, (int, tubes.process.process, tubes.ssh.ssh_channel)):
        log.error("Use r2.attach() to debug a running process")
...
def binary():
    ...
    Returns:
        str: Path to the appropriate ``r2`` binary to use.

    Example:

        >>> r2.binary() # doctest: +SKIP
        '/usr/bin/radare2'
    """
    r2 = misc.which('pwntools-r2') or misc.which('r2')
    if not r2:
...
@LocalContext
def attach(target, r2script = None, exe = None, need_ptrace_scope = True, r2_args = None, ssh = None, sysroot = None):
    ...
    Arguments:
        ...
        r2script(:obj:`str` or :obj:`file`): r2 script to run after attaching.
        exe(str): The path of the target binary.
        arch(str): Architechture of the target binary.  If `exe` known r2 will
          detect the architechture automatically (if it is supported).
        r2_args(list): List of additional arguments to pass to r2.
        sysroot(str): Foreign-architecture sysroot, used for QEMU-emulated binaries
            and Android targets.

    Returns:
        PID of the r2 process (or the window which it is running in).
    ...
    # if r2script is a file object, then read it; we probably need to run some
    # more r2 script anyway
    if isinstance(r2script, file):
        with r2script:
            r2script = r2script.read()

    # enable r2.attach(p, 'continue')
    if r2script and not r2script.endswith('\n'):
        r2script += '\n'
    ...
    if   isinstance(target, (int, long)):
        ...
        r2script = 'shell rm %s\n%s' % (tmpfile, r2script)
        shell.upload_data(r2script or '', tmpfile)
        ...
        if shell.keyfile:
            cmd += ['-i', shell.keyfile]
        cmd += ['r2 -e dbg.exe.path=%r -d %s -i "%s"' % (target.executable,
                                       target.pid,
                                       tmpfile)]
    ...
    #r2script = pre + (r2script or '')
    r2script = (r2script or '')
    if r2script:
        tmp = tempfile.NamedTemporaryFile(prefix = 'pwn', suffix = '.r2',
                                          delete = False)
        log.debug('Wrote r2 script to %r\n%s' % (tmp.name, gdbscript))
        r2script = '!rm %s\n%s' % (tmp.name, gdbscript)
    ...
    r2_pid = misc.run_in_new_terminal(cmd)
...
def find_module_addresses(binary, ssh=None, ulimit=False):
    log.error('This method hasn\'t been implemented yet.')
    pass
    """
...
def version(program='r2'):
    """Gets the current GDB version.

    Note:
        Requires that GDB version meets the following format:

        ``radare2 2.9.0-git 19113 @ darwin-x86-64 git.2.8.0-93-ga095f6f3f``
    ...
    expr = r'radare2 ([^-]+)(-git)? (\d+ [@]+ [^ ]+) (.+)'

    with tubes.process.process([program, '-v'], level='error') as r2:
        version = r2.recvline()

    versions = re.search(expr, version).group(1)
