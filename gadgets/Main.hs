...
disas :: Elf.ElfSection -> [Instruction]
disas sec =
  case runIdentity $ disassembleListWithConfig conf
       $ B.unpack $ Elf.elfSectionData sec of
    -- Left err  -> err
    Right insts -> insts
    where
      conf =
        ...
        , confStartAddr      = fromIntegral $ Elf.elfSectionAddr sec
        }
